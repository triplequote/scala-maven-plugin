...
public abstract class HydraLicenseMojo extends AbstractMojo {
    ...
    /**
     * @parameter property="hydra.license.activationServerUrl"
     *            default-value="https://activation.triplequote.com/algas/"
     */
    protected String activationServerUrl;

    /**
     * @parameter property="hydra.license.store"
     *            default-value="${user.home}/.triplequote/hydra.license"
     */
        ...
        if (session.getCurrentProject() == session.getTopLevelProject()) {
            try {
                ...
                throw new MojoExecutionException(
                    String.format("Failed to parse URL %s : %s", activationServerUrl, e.getMessage()), e);
            } catch (IOException e) {
    ...
    private void operateOnLicense() throws MojoFailureException, MojoExecutionException, IOException {
        HydraLicenseStore store = new HydraLicenseStore(new URL(activationServerUrl), Paths.get(licenseStore));
    ...
    /**
     * Perform the license operation on the given store. Error checking is performed
     * by caller.
     * 
     * @param store
     *            The store that contains Hydra licenses
     * @return A license that was activated/deactivated. Status can be checked on
     *         the license for error reporting
     * @throws IOException
     */
